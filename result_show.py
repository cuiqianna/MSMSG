...
#         #subgraph_folder = os.path.join(file_folder, "subgraph/merge")
#
#         for j in range(1):
#             str_seg = seg_str_set[j]
#
#             index_file = os.path.join(index_folder, str(2000) + "_new.csv")
#             index = np.loadtxt(open(index_file), delimiter=',', dtype=np.uint32, skiprows=0)
#             subgraph_folder = os.path.join(file_folder, "pre_seg/TC/2000" )
#
...
#                         img_new[h, w, :] = 0
#
#             component_save_file = os.path.join(component_folder, str(2000) + "_component.csv")
...
#             merge_com_num = len(long_com)
#
#             tau_set_name = os.path.join(index_folder,  "_tau_optimal.csv")
#             tau_optimal_set = read_tau(tau_set_name)
#
#             for k in range(merge_com_num):
#
#                 cur_tau = tau_optimal_set[k]
#
#                 tem_cur_tau = '%.2f' % cur_tau
#
#                 ii = long_com[k]  #所有component中需要融合的com索引
#                 cur_com = com_read[ii]
#                 x_min, x_max, y_min, y_max, cur_index = component_index(cur_com, index)
...
#
#                 tem_com = '%d' % (ii +1)
#                 cur_seg_name = os.path.join(subgraph_folder,  tem_com + "_" + tem_cur_tau + ".csv")
#                 seg_index = np.loadtxt(open(cur_seg_name), delimiter=',', dtype=np.uint32, skiprows= 0)
#
#                 if (cur_height == 1 or cur_width == 1):
#                     seg_index2 = np.zeros(shape= (cur_height, cur_width), dtype=np.uint32)
...
#                 idx_num = new_idx_num2
#
#             new_index_name = os.path.join(index_folder, str(2000) + "_result.csv" )
#             np.savetxt(new_index_name, index_new, delimiter=',', fmt='%d')
#
#             new_image_name = os.path.join(index_folder, str(2000) + "_result.jpg" )
#
#             new_image = new_image_show(img_new, index_new)

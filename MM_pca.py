...
#
#     time_matrix = np.zeros(shape=(3,5), dtype= np.float32)
#
#     for i in range(1,2):
#         cur_str = file_str[i]
#
...
#
#         image_folder = os.path.join(file_folder, "GMM")
#
#         image_save_folder = os.path.join(file_folder, "MM")
#
#         image_file_name = os.path.join(image_folder, "GMM_LAB.jpg")
#         image = plt.imread(image_file_name)
#
#         initial_size = 3
#         size_increment = 2
#         #max_size = superpixel_size_compute(index, count)
#         max_size = 3
#         tem = '%d' % max_size
#
#         #print(cur_str, ":", str_seg, ":", "max_size--", max_size)
#
#         #start = time.time()
#
#         open_pca_matrix = open_pca(image, initial_size, size_increment, max_size)
#         cur_open_file = os.path.join(image_save_folder, tem + "_open_pca.jpg")
#         cur_open_csv = os.path.join(image_save_folder, tem + "_open_pca.csv")
#         plt.imsave(cur_open_file, open_pca_matrix, cmap='gray')
#         np.savetxt(cur_open_csv, open_pca_matrix, delimiter= ',', fmt = '%.4f')
#
#         open_image = im_show(image, open_pca_matrix)
#
#         close_pca_matrix = close_pca(open_image, initial_size, size_increment, max_size)
#         #
#         cur_close_jpg = os.path.join(image_save_folder, tem + "_close_pca.jpg")
#         plt.imsave(cur_close_jpg, close_pca_matrix, cmap='gray')
#
#         cur_close_csv = os.path.join(image_save_folder, tem + "_close_pca.csv")
#         np.savetxt(cur_close_csv, close_pca_matrix, delimiter=',', fmt='%.4f')
#
#
